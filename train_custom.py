...
if __name__ == '__main__':
    ...
    if args.build_vocab and not os.path.isfile('vocab_%s.pkl' % args.exp_name):
        print("Building vocabulary")
        text_field.build_vocab(train_dataset, val_dataset, min_freq=5)
        pickle.dump(text_field.vocab, open('vocab_%s.pkl' % args.exp_name, 'wb'))
    elif args.build_vocab and os.path.isfile('vocab_%s.pkl' % args.exp_name):
        text_field.vocab = pickle.load(open('vocab_%s.pkl' % args.exp_name, 'rb'))
    else:
        text_field.vocab = pickle.load(open('vocab.pkl', 'rb'))

    # Model and dataloaders
    encoder = MemoryAugmentedEncoder(3, 0, args.d_in, d_ff=args.d_in, attention_module=ScaledDotProductAttentionMemory, attention_module_kwargs={'m': args.m})
    ...
    if args.resume_last or args.resume_best:
        ...
        if os.path.exists(fname):
            ...
            try:
                ...
                middle_time = None
            print('Resuming from epoch %d, validation loss %f, best validation cider %f, patience %d' % (data['epoch'], data['val_loss'], data['best_cider'], data ['patience']))

    print("Training starts at ", start_time.strftime("%H:%M:%S, %d/%m/%Y"))
    for e in range(start_epoch, start_epoch + 100):
        ...
        if patience == 5:
            if args.scst and not use_rl:
                use_rl = True
                switch_to_rl = True
                patience = 0
                optim = Adam(model.parameters(), lr=5e-6)
                middle_time = datetime.datetime.now()
                print("Switching to RL at ", middle_time.strftime("%H:%M:%S, %d/%m/%Y"))
                print("Total xe time: ", middle_time - start_time)
            else:
                end_time = datetime.datetime.now()
                print('Patience reached at ', end_time.strftime("%H:%M:%S, %d/%m/%Y"))
                if args.scst:
                    print('Total rl time: ', end_time - middle_time)
                print('Total training time: ', end_time - start_time)
                exit_train = True
        elif args.random == True and e == start_epoch + 14:
            end_time = datetime.datetime.now()
            print('Maximum number of epochs reached at ', end_time.strftime("%H:%M:%S, %d/%m/%Y"))
            print("Total xe time: ", end_time - start_time)
            exit_train = True
        ...
        if switch_to_rl and not best:
            ...
            print('Resuming from epoch %d, validation loss %f, and best cider %f' % (data['epoch'], data['val_loss'], data['best_cider']))
        
        try:
            mid_time = middle_time
        except:
            mid_time = None
        torch.save({
...
# python train_custom.py --exp_name exp_name --features_path features_path (--d_in d_in) (--random) (--scst) (--build_vocab) 